...
- AÇIK SORUN (31 Ağustos 2026): iOS'ta hem haftalık hem yıllık abonelik satın alma başarısız, Android gerçek cihazlarda çalışıyor. Dart tarafı platform bağımsız; şüpheliler App Store Connect sözleşme/ürün tanımı, RC paketlerine App Store ürünü bağlanmamış olması, StoreKit 2 test kurulumu (gerçek cihaz + ücretli team + Sandbox hesabı; simülatör için `.storekit` yok). `main.dart` RC init hatasını yalnızca `debugPrint`'liyor ve `RevenueCatService._selectPackage` eşleşme yoksa sessizce ilk pakete düşüyor — teşhis için önce konsoldaki `[Purchases]` / `RevenueCat purchase platform error` satırlarına bak.
