...
| 15 | `date_of_birth_value_object.dart` | **Hata değil, ürün kararı.** Alanın başlığı "Doğum Tarihi (Opsiyonel)", `createUser` guard'ı doğum tarihini kontrol etmiyor ve asistan boş değer için "Bilinmiyor" kullanıyor; ürün sahibiyle teyit edilerek boş değer **geçerli** bırakıldı (`DateOfBirthEmpty` sınıfı ileride zorunlu yapılırsa kullanılmak üzere duruyor). Dördüncü turda profil düzenleme ekranındaki `DateOfBirthTextField`'a `displayError` bağlandı; artık hatalı bir tarih girildiğinde mesaj görünüyor. |
...
## 11. Ölü kod temizliği (31 Ağustos 2026, beşinci tur)

`main.dart`'tan import/export zinciriyle erişilebilirlik analizi, tip/metot referans sayımı ve yorum-blok taraması yapıldı. Hiçbir yerden referans almayan şu öğeler kaldırıldı (71 dosya, −1.771 satır): erişilemeyen `lib/ui/dhikr/create_dhikr_by_mood/` kopyası; hiç örneklenmeyen `DhikrViewModel`, `AuthViewModel`, `FlowViewModel`, `PostDetailViewModel`; tamamı yorum olan `sign_up_use_case.dart`; boş barrel/placeholder dosyaları; kullanılmayan widget'lar (`AppResendCodeButton`, `BaseColumn`, `BaseKeepAlive`, `CircleImageWithUrl`, `InformationalDialog`, `LogOutButton`, `NameTextField`, `SurnameTextField`, `PaginatedBuilder`, `PurchaseBenefitTile`, `UserProfileUtils`, `AppInitializingView`); 13 kullanılmayan router yardımcısı; 5 yorum satırına alınmış kod bloğu; 8 kullanılmayan `AppStrings` sabiti; başka projeden kalma `Makefile`; kullanılmayan `cupertino_icons` bağımlılığı. Silinen sınıflara ait 35 test de kaldırıldı. Doğrulama: `flutter analyze` (yalnızca 6 eski `info`), `flutter test` 1040 geçen / 0 başarısız.

Karar bekleyenler (kullanılmıyor ama "planlanmış özellik" olabilir): `AuthRepository.reauthenticate` zinciri (hesap silme öncesi yeniden kimlik doğrulama — Firebase bunu gerektirir), `PostRepository`'nin 5 `UnimplementedError` stub'ı ve `cancelDhikrReminderNotification` stub'ı, `Emotion` enum'u, `DhikrRepository.getDhikrsByGroupId/getAllDhikrsFromFirestore`, `HiveService.saveAll/getAllAsMap/getAllKeys`, `Permission.toUserFriendly*`, `PermissionStates.fromPermission`, `DhikrUseCase.deviceHasConnection`, `PlaceSelectorViewModel`'in dört yardımcı metodu, `NavigationBarViewModel.currentTabIndex`, `PurchaseViewModel.purchaseWeekly/purchaseYearly`, hiç çalıştırılmayan `FetchUserViewModel.fetchCurrentUser`, içi boş `UserProfileViewModel`. Not: `PurchaseViewModel.restorePurchases` çağrısı `purchase_view.dart`'ta yorum satırında — "Satın alımları geri yükle" App Store için zorunludur, yeniden etkinleştirilmesi önerilir.
